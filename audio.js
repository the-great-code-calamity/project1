/** This is an audio worklet processor pulled from https://webaudio.github.io/web-audio-api/#the-bitcrusher-node
 *  for the purpose of converting 32 to 16 bit. Seriously, if I wanted to do 
 *  math I would have joined the physics club. 
 * 
 */
        ...
        if (bitDepth.length > 1) {
            ...
            for (let channel = 0; channel < output.length; ++channel) {
                for (let i = 0; i < output[channel].length; ++i) {
                    ...
                    if (this._phase >= 1.0) {
                        this._phase -= 1.0;
                        // console.log(`EE -> ${input[channel]}  :: Channel ${channel} -> i ${i}`);
                        this._lastSampleValue =
                            step * Math.floor(input[channel][i] / step + 0.5);
                    }
                    output[channel][i] = this._lastSampleValue;
